...
rdd_poems.persist()
rdd_poems = rdd_poems.partitionBy(2, partitionFunc=hash_by_author)
rdd_poems.persist()
rdd_novels = rdd_novels.join(rdd_reviews)
rdd_novels.persist()
rdd_novels = rdd_novels.map(lambda x: [x[1][0], (x[0], x[1][1])])
rdd_novels.persist()
rdd_novels = rdd_novels.partitionBy(2, partitionFunc=hash_by_author)
rdd_novels.persist()
