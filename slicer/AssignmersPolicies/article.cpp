...
namespace Article
{
    ...
    void MakeAssignment(Assignment &assignment_)
    {
                    ...
                    for (ssize_t k = 0; k < (ssize_t)hot.size(); ++k)
                    {
                        for (ssize_t t = 0; t < (ssize_t)hot[k].size(); ++t)
                        {
                            if (assignment.Get(i, j).second == assignment.Get(k, t).first)
                            {
                                ...
                                if (hot[i][j] + hot[k][t] < average_temperature_sum / ranges_cnt)
                                {
        ...
        if (cool_first.task != -1 && cool_second.task != -1)
        {
            ...
            while (ranges_cnt / hotness.size() >= 50 &&
                   hotness[cool_first.task][cool_first.ind] + hotness[cool_second.task][cool_second.ind] < average_temperature_sum / std::max(1ll, ranges_cnt) && merge_direction != Direction::DontMove && churn * 100 < MAX_KEY)
            {
        ...
        auto max_task_disbalance_change_strct = [](std::vector<std::vector<double>> hot, Assignment &assignment)
        {
            ...
            for (size_t range_ind = 0; range_ind < hot[ind_max].size(); ++range_ind)
            {
                ...
                double diff_in_hotness = std::max(
                    max_weight - std::max(max_weight - hot[ind_max][range_ind] / 2, min_weight + hot[ind_max][range_ind] / 2),
                    max_weight - std::max(max_weight - hot[ind_max][range_ind], min_weight + hot[ind_max][range_ind]));
        ...
        while (get_avg_ranges_per_task(hotness) < 150 && hotness[ind_to_split.task][ind_to_split.ind] > 2 * get_avg_slice(hotness))
        {
